...
import { ArrowLeft, Eye, Clock, CheckCircle2, XCircle, User, Mail, Phone, MapPin, CreditCard, Calendar, Briefcase, AlertCircle, Bell, ChevronRight, Trash2, ClipboardCheck, ClipboardList } from 'lucide-react';
...
const AdminApplications = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'provider_applications' },
        (payload) => {
  ...
  const updateStatus = async (id: string, status: string) => {
    ...
    try {
      ...
      if (session) {
        ...
        await fetch(
          `https://${projectId}.supabase.co/functions/v1/notify-provider-status`,
          {
            method: 'POST',
            headers: {
  ...
  const openDetails = (app: ProviderApplication) => {
    setSelected(app);
    setSheetOpen(true);
  };
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selected.status === 'pending' && (
                    <div className="flex gap-2 ml-auto">
                      <Button size="sm" onClick={() => updateStatus(selected.id, 'approved')} disabled={updating}>
                        Approve
                      </Button>
                      <Button size="sm" variant="destructive" onClick={() => updateStatus(selected.id, 'rejected')} disabled={updating}>
                        Reject
                      </Button>
                    </div>
                  )}
